...
| **D2** | **Idioma de la UI** | **Inglés**, con i18n desde el día 1 (`react-i18next`) pero un solo locale | La spec fija código y commits en inglés. Una UI en inglés amplía la audiencia; el andamiaje de i18n permite añadir español después sin refactor. |
...
- **Fase 3 — Aprendizaje.** Lecciones guiadas · modo reto con **validación autoritativa en servidor** usando el mismo `@qsim/core` (riesgo #5) · tablas de posiciones · embeds con CSP restrictiva.
...
| **B3** | Confirmar el **idioma de la UI** (D2) | Antes de M0.5 | Recomiendo inglés con i18n listo. |
...
1. **B1** — instalas Node 22
2. **B2** — confirmas D1–D6
3. Yo ejecuto **M0.0** (andamiaje) y abro el primer PR
